...
\description{
  ...
  files. The package \code{eatModel} was originally created to allow for more fail-save, less cumbersome
  ...
  \itemize{
    ...
    \item \code{\link{runModel}} needs the output generated by \code{\link{defineModel}} and simply starts the
          \sQuote{estimator} (e.g. Conquest, TAM, or mirt)
  ...
  may be estimated simultaneously. The function \code{\link{splitModels}} allows to allocate the analyses
  to several cores. See the examples of \code{\link{splitModels}} for further details. The help page of
  \code{\link{defineModel}} includes a variety of examples which are derived from the context of the
  ``IQB Laendervergleich'' studies.
  
  Basically, \code{eatModel} is useful for Conquest analyses which are called from R. Alternatively,
  the R packages \code{TAM} and \code{mirt} allow to estimate parameters of a variety of IRT models
  in an R environment. \code{eatModel} allows all models to be estimated in \code{TAM} or \code{mirt}
  likewise. The \code{software} argument in the \code{\link{defineModel}} function allows to choose
  ...
  support of 2pl and partial credit models using the \code{TAM} and \code{mirt} packages and several
