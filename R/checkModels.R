...
checkContextVars <- function(x, varname, type = c("weight", "DIF", "group", "HG"), itemdata, suppressAbort = FALSE, internal = FALSE, renam)   {
...
checkBGV <- function(allNam, dat, software, remove.no.answersHG, remove.vars.DIF.missing, namen.items.weg, remove.vars.DIF.constant, renam){
     ...
     ### Hintergrundvariablen (conditioning model)
            if(length(allNam[["HG.var"]])>0)    {
               ...
               if(!all(varClass)) {
                  ...
                  dat <- data.frame ( dat, ind, stringsAsFactors = FALSE )      ### ergaenze neue Variablen im Datensatz
               }
     ...
     ### Gruppenvariablen
            if(length(allNam[["group.var"]])>0)  {
                group.info <- lapply(allNam$group.var, FUN = function(ii) {checkContextVars(x = dat[,ii], varname=ii, type="group", itemdata=dat[,allNam[["variablen"]], drop = FALSE], internal=TRUE, renam=renam)})
     ...
     ### DIF-Variablen
            if(length(allNam[["DIF.var"]])>0)  {
                dif.info <- lapply(allNam[["DIF.var"]], FUN = function(ii) {checkContextVars(x = dat[,ii], varname=ii, type="DIF", itemdata=dat[,allNam[["variablen"]], drop = FALSE], internal = TRUE, renam=renam)})
...
checkItemConsistency <- function(dat, allNam, remove.missing.items, verbose, removeMinNperItem, minNperItem, remove.constant.items, model.statement, software, renam){
     ...
     ### sind die responses numerisch bzw. stehen da Ziffern drin? (notfalls sowas wie as.character(1) )
          ...
          if (length(noZahl) > 0 ) {
              ...
              cat(paste0("Warning: Found ", length(noZahl), " non-numeric value(s) in ",length(itemNoZ)," of ",length(allNam[["variablen"]])," items: '", paste( optionalRenam(itemNoZ,renam), collapse= "', '"), "'. Non-numeric values: '", paste( unique(datL[noZahl,"value"]), collapse= "', '"), "'.\n"))
          }
     ...
     ### identifiziere Items ohne jegliche gueltige Werte
          if(length(n.mis) >0) {
             ...
             cat(paste0("Warning: ",length(n.mis), " testitems(s) without any values: ",paste0(weg[["mess"]], "'", paste( optionalRenam(names(n.mis),renam) , collapse="', '"),"'"), "\n"))
          }
     ### identifiziere Items mit Anzahl gueltiger Werte < minNperItem
          ...
          if(length(below) > 0 ) {
             ...
             namen.items.weg <- c(namen.items.weg, weg[["niw"]])
             cat(paste0("Warning: ",length(below), " testitem(s) with less than ", minNperItem, " valid responses: ", paste0(weg[["mess"]], "'", paste(optionalRenam(names(below),renam), collapse="', '"),"'"), "\n"))
          }
     ### identifiziere konstante Items (Items ohne Varianz)
          ...
          if(length(constant) >0) {
             ...
             cat(paste("    Item '", optionalRenam(names(constant),renam), "', only value '", uniqueVal, "' occurs: ", nVal, " valid responses.", collapse="\n", sep="")); cat("\n")
          }
