compareDefineModelObjects <- function (ref, tar, round = TRUE, digits = 3) {
       ...
       if (sum(mult) == 2) {                                                    ### multiple object: jetzt erstmal schauen, ob die sortierung uebereinstimmt
           ...
           ord  <- 1
       }                                                                        ### comparable objects, die zu vergleichenden Objekte erstellen
       co   <- lapply( ord, FUN = function(i) {                                 ### das braucht man dann, wenn ein einziger 'defineModel'-Aufruf bspw. 16 Modelle
               if(inherits(i, "character")) {                                   ### fuer 16 Bundeslaender enthaelt. Dann wird fuer jedes der 16 Laender verglichen
                   nams <- sapply(list(ref, tar), FUN = function (x) {which(sapply(x, FUN = function (y) {y[["analysis.name"]]==i}))})
                   dats <- list(ref = ref[[nams[1]]], tar = tar[[nams[2]]])     ### wenn Analysen ohne 'split.models' verglichen werden, ist 'co' eine Liste der Laenge 1, sonst
               }  else  {                                                       ### bspw. eine Liste der Laenge 16
                   dats <- list(ref=ref, tar=tar)
               }})
       vgl  <- lapply(co, FUN = function (v) {                                  ### jetzt jeder einzelne Vergleich
               stopifnot(length(v) == 2)                                        ### erstmal nur das vergleichen, was einfache strings sind
...
compareQmatrices <- function (qm1, qm2){
       if ( ncol(qm1) != ncol(qm2) || !all(sort(colnames(qm1)) == sort(colnames(qm2))) ) {
            ...
            if( ncol(qm1) == ncol(qm2) ) {                                      ### versuche Spaltennamen zu harmonisieren
...
### wenn es single ist, dann eine liste draus bauen
# save (modKon, modDefSW, file = "c:/diskdrv/Winword/Psycho/IQB/temp/tempdata_nicht_loeschen2.rda", compress="bzip2")
# foo <- compareDefineModelObjects(ref=modKon, tar=modDefSW)
