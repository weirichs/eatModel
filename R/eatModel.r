...
equat1pl<- function ( results , prmNorm , item = NULL, domain = NULL, testlet = NULL, value = NULL, excludeLinkingDif = TRUE, difBound = 1, iterativ = FALSE, method = c("Mean.Mean", "Haebara", "Stocking.Lord", "robust", "Haberman"),
    ...
    ### items muessen unique sein
                               ...
                               if (method == "Haberman") { cat(paste("          estimation method: " , recode(estimation,"'OLS'='ordinary least squares'; 'BSQ'='bisquare weighted regression'; 'HUB'='regression using Huber weights'; 'MED'='median regression'; 'LTS'='trimmed least squares'; 'L1'='median polish'; 'L0'='minimizing number of interactions'"), "\n",sep="")) }
...
transformToBista <- function ( equatingList, refPop, cuts, weights = NULL, defaultM = 500, defaultSD = 100, roman = FALSE, vera = TRUE ) {
    ...
    ### 'refPop' Informationstabelle bauen
       ...
       itempars   <- do.call("rbind.fill", lapply ( modN, FUN = function ( x ) { x[["itempars"]]}))
    ...
    ### erstmal zusaetzliche Informationen (Anzahl nodes etc.) gewinnen
         ...
         ret  <- lapply(such, FUN = function ( su ) {
                 ...
                 if(length(indm) != 1) {
                    ...
                    met <- removePattern(substr(cqc[indm], pos1+1, pos2-1), paste0(su,"="))
                 }
         ...
         pdf(file = file.path ( path, paste0(analysis.name, "_dev.pdf")), width = 10, height = 7.5)
...
desk.irt <- function(daten, itemspalten, na=NA,percent=FALSE,reduce=TRUE,codebook=list(datei=NULL,item=NULL,value=NULL,lab=NULL, komp=NULL), quiet = FALSE ) {
            if( !"data.frame" %in% class(daten) ) {stop("'daten' must be of class 'data.frame'.\n")}
             ...
             results        <- lapply(1:ncol(daten), FUN=function(ii) {
                               res.i <- tablePattern(x=daten[,ii], pattern=specific.codes[[ii]]$value)
...
plotDevianceConquest <- function ( logFile, omitUntil = 1, reverse = TRUE, change = TRUE ) {
           ...
           mat  <- data.frame ( iter = 1:length(ind), as.numeric(crop(substring(input[ind], 13))))
           if(omitUntil>0)  {
