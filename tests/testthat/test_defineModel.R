...
modsDef_Geschl_Conquest  <- defineModel(dat = findat, splittedModels = modSplit,id = "thnum", HG.var = "thdiff", remove.failures = TRUE, DIF.var = "Geschl", dir = tempdir())


###
load(system.file("extdata", "pcm", "input.rda", package = "eatModel"))
define3d <- defineModel(dat = datAufbereitet, items = qmatrix[,"item_neu"], id = "Pseudonyme",HG.var = "Note.y", model.statement = "item + item*step", nodes = 50,
...
load("df_example2.rda")
def <- defineModel(dat = datSel, items = intersect(qmat[,"item"],colnames(datSel)), compute.fit = FALSE, id = "IDSTUD", remove.constant.items = TRUE,method = "montecarlo", nodes = 8000,weight.var = "totwgt_NAW",
...
# single
   ...
   defDif4 <- defineModel(dat = datW, id = "idstud", items = grep("^T", colnames(datW), value=TRUE), software="conquest", DIF.var = "sex", dir = tempdir(), analysis.name = "DIF")
   runDif4 <- runModel(defDif4)
   resultsDif4 <- getResults(runDif4)
...
# multiple
   ...
   defDif4 <- defineModel(dat = datW, id = "idstud", splittedModels = split, software="conquest", DIF.var = "sex", dir = tempdir(), analysis.name = "DIF")
   runDif4 <- runModel(defDif4)
   resultsDif4 <- getResults(runDif4)
   itemsDif4   <- itemFromRes(resultsDif4)
