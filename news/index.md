...
- minor bug fix in
  ...
  for TAM: tam function selection between 1pl and 2pl is correct now
- Use
  [`mirt::traditional2mirt()`](https://philchalmers.github.io/mirt/reference/traditional2mirt.html)
  for transformation of item parameters.
- implement 2pl transformation for item parameters according to a
  probability of 62.5%
...
- add function
  [`getTrafo()`](https://weirichs.github.io/eatModel/reference/getTrafo.md)
  for extraction of IQB transformation rules from data base
...
- bugfix in
  [`runModel()`](https://weirichs.github.io/eatModel/reference/runModel.md)
  for partial credit in mirt: column identification of irtmodel via
  number instead colname
...
- added new test file test_defineModel.R, copied TAM and Conquest
  examples from .Rd file (examples 8, 8.1), renamed objects for
  comparison
...
- WLEs in multidimensional models for mirt: account for missings on sub
  dimensions
...
- [`multiEquatError()`](https://weirichs.github.io/eatModel/reference/multiEquatError.md):
  rewritten: tests added, help files adapted
- remove assertions in \`fixSlopeMat(): In the assertions, the columns
  must not be selected according to order, as this can vary.
- remove recursive calls in
  [`defineModel()`](https://weirichs.github.io/eatModel/reference/defineModel.md):
  new internally needed function `defineModelSingle()`

## eatModel 0.8.52 \[Mar 2025\]

- Fix in
  [`multiEquatError()`](https://weirichs.github.io/eatModel/reference/multiEquatError.md):
  The old function lacked a check if the testlets are constant. In
  addition, the comparison of the original and chained linking errors
...
- The functions are no longer contained in a single file (eatModel.r),
  but are grouped according to their intended use.
- `getConquestVersion()`,
  [`simEquiTable()`](https://weirichs.github.io/eatModel/reference/simEquiTable.md):
  new assertions (input validation)
- getResults.rd: change file encoding from ISO-8859-1 to UTF-8
- `checkQmatrixConsistency()`: moved checks from
  [`splitModels()`](https://weirichs.github.io/eatModel/reference/splitModels.md)
  in here (because the function is also called by defineModel
- added checkmate checks for
  [`defineModel()`](https://weirichs.github.io/eatModel/reference/defineModel.md)
- remove problematic
  [`eatTools::gsubAll()`](https://weirichs.github.io/eatTools/reference/gsubAll.html)
  from
...
- replace deprecated
  [`memory.limit()`](https://rdrr.io/r/utils/memory.size.html) with
...
- [`get.shw()`](https://weirichs.github.io/eatModel/reference/get.shw.md):
  Conquest only allows variable names with a maximum of 11 characters.
  If the variable name is exactly 11 characters long, the space between
  the variable name and the item parameter value is missing in the ASCII
  file. In this case, the columns could not be read correctly because
  this was done by identifying spaces in the ASCII file. This bug has
  been fixed here.

## eatModel 0.8.21 \[Feb 2023\]

- Bug fix for rounding in
  [`simEquiTable()`](https://weirichs.github.io/eatModel/reference/simEquiTable.md):
  Depending on the number of decimal places to be displayed, a less
  ambiguous rounding method is now selected, which makes it possible to
  determine exactly which proficiency level an item falls into when the
  transformed item parameter lies on the boundary between proficiency
  levels.
- add warnings in
  [`equat1pl()`](https://weirichs.github.io/eatModel/reference/equat1pl.md)
  if dimension names do not correspond to refPop’s first column names
- add seed argument in
  [`getResults()`](https://weirichs.github.io/eatModel/reference/getResults.md)
  (for infit computation)
- added assertions in
  [`replaceLinkingError()`](https://weirichs.github.io/eatModel/reference/replaceLinkingError.md):
  verbose, digits
- add package sticker
- customize model statement in
  [`defineModel()`](https://weirichs.github.io/eatModel/reference/defineModel.md)
  for Conquest: The function checks whether the variable names in the
  `model` statement are also included in the dataset. However, this does
  not apply to reserved names such as “item,”, “step,” etc.
...
- add
  ...
  which facilitates quality checks by comparing two objects returned by
  [`defineModel()`](https://weirichs.github.io/eatModel/reference/defineModel.md)
...
- provide optional long format return in
...
- The function `mergeAttr()` is being moved from the eatModel package to
  the eatTools package.

## eatModel 0.8.0 \[Feb 2022\]

- add function
  [`checkContextVars()`](https://weirichs.github.io/eatModel/reference/checkContextVars.md)
  which checks whether some types of context vars, i.e. group, DIF and
  weighting variables, are consistent with item variables. The function
  is mainly used for internal consistency checks.

## eatModel 0.7.57 \[Feb 2022\]

- add function
  [`regcoefFromRes()`](https://weirichs.github.io/eatModel/reference/regcoefFromRes.md)
  which lists the regression parameters of the conditioning model from
  the results object.

## eatModel 0.7.51 \[Feb 2022\]

- [`transformToBista()`](https://weirichs.github.io/eatModel/reference/transformToBista.md)
  now gives linking errors in a data.frame which is appropriate for
  eatRep
...
- Bug fix in
  [`transformToBista()`](https://weirichs.github.io/eatModel/reference/transformToBista.md):
  drawing PVs failed when items are only partially linked

## eatModel 0.7.47 \[Dec 2021\]

- add examples for trend analyses with 3 measurement occasions
- [`defineModel()`](https://weirichs.github.io/eatModel/reference/defineModel.md):
  add output information about students with all items solved or all
  items failed
...
- Extracting the recursive function part from
  [`defineModel()`](https://weirichs.github.io/eatModel/reference/defineModel.md)
  which is now provided in a new function `doAufb()`
- add
  [`multiEquatError()`](https://weirichs.github.io/eatModel/reference/multiEquatError.md)
  for linking of three measurement
...
- [`wleRelFromRes()`](https://weirichs.github.io/eatModel/reference/wleRelFromRes.md)
  adds wle reliability to results object, also for multidimensionality
  models
...
- If it cannot be found on the network drives, the user will be prompted
  to specify the location of the Conquest console the first time it is
  needed.
...
- set compatibility to eatRep 0.13.0: replace
  [`jk2.mean()`](https://weirichs.github.io/eatRep/reference/repMean.html)
  by
  [`repMean()`](https://weirichs.github.io/eatRep/reference/repMean.html)

## eatModel 0.7.24 \[Aug 2020\]

- Bug fix: Importing WLEs from Conquest now also works for
...
- [`transformToBista()`](https://weirichs.github.io/eatModel/reference/transformToBista.md)
  calculates the mean and standard deviation of the reference population
  using eatRep methods when the given sample is drawn from the norm
  population and is not to be equated.

## eatModel 0.7.11 \[Feb 2020\]

- Add Haberman linking to
  [`equat1pl()`](https://weirichs.github.io/eatModel/reference/equat1pl.md)

## eatModel 0.7.10 \[Jan 2020\]

- Add robust methods to
  [`equat1pl()`](https://weirichs.github.io/eatModel/reference/equat1pl.md)
  linking procedures

## eatModel 0.7.8 \[Nov 2019\]

- Based on an idea by Johannes Schult, the
  [`simEquiTable()`](https://weirichs.github.io/eatModel/reference/simEquiTable.md)
  function now uses methods from Ivailo Partchev’s irtoys package to
  generate the item parameter equivalence table more quickly and
  efficiently.

## eatModel 0.7.7 \[Nov 2019\]

- Temporary storage of console messages that cannot be displayed
  immediately in the case of multicore. They are stored as strings and
  displayed on the console once the multicore analysis is complete.

## eatModel 0.7.6 \[Nov 2019\]

- Bug fix for recursive calls to
  [`defineModel()`](https://weirichs.github.io/eatModel/reference/defineModel.md)
  in cases involving multiple models

## eatModel 0.7.4 \[May 2019\]

- compatibility issues: Add functionality that allow the ID variable to
  be specified manually when output generated by earlier package
  versions needs to be re-read by newer package versions.
