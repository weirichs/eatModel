...
\description{
  ...
  simultaneously. The function \code{splitModels} allows to allocate the analyses to several cores.
  See the examples of \code{splitModels} for further details. The help page of \code{defineModel}
  ...
  was the first study where \code{eat} has been used intensely. In 2012, the functionality of the
  \code{eat} package has grown and therefore was partitioned into several small ``sub packages''---by
  name \code{eatPrep} for data preparation, \code{eatTools} for several auxiliary functions,
  \code{eatRest} for Rasch modeling, \code{eatRep} for replication methods, \code{eatDesign}
  for definition and modification of design properties. The further development of \code{eatRest}
  was expired because the package turned out to be enigmaticly written which leads to undebugable
  problems, especially with the ardently loved results structure. Thus, \code{eatModel} is the
  second attempt. Its functionality is closely related to \code{eatRest} but adds some new
