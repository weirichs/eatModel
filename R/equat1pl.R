equat1pl<- function ( results , prmNorm , item = NULL, domain = NULL, testlet = NULL, cat=NULL, value = NULL, excludeLinkingDif = TRUE, difBound = 1, iterativ = FALSE, method = c("Mean.Mean", "Haebara", "Stocking.Lord", "robust", "Haberman"),
...
### checking/assering the arguments --------------------------------------------
  ...
  lapply(c(b_trim, lts_prop), checkmate::assert_numeric, len = 1)
    ...
    ### Equating: baue 'befuelltes' Rueckgabeobjekt
                ...
                prmNorm<- eatTools::makeDataFrame(prmNorm, verbose=FALSE)
                allN   <- checkItemParLists(prmNorm =prmNorm, item = item, domain = domain, testlet = testlet, value = value, dims=dims, cat=cat)
    ...
    ### sollen nur fuer diesen Kompetenzbereich hier keine Testlets beruecksichtigt werden. Dafuer wird INNERHALB DER SCHLEIFE allN[["testlet"]] auf NULL zurueckgesetzt
                               ...
                               if (!is.null(allN[["testlet"]])) {
                                   ...
                                   if(length(which(is.na(mrge[,allN[["testlet"]]]))) > 0) {
                                       ...
                                       allN[["testlet"]] <- NULL                ### obere Zeile: message soll erst spaeter auf die Konsole, wird hier nur erzeugt
                                   }
                               }
    ### wenn es partial credit ist, wird ein neuer Item-Index gebildet aus Item und category
                               if("category" %in% colnames(prmDim) && max(as.numeric(eatTools::removeNonNumeric(prmDim[,"category"]))) > 1)  {
                                  prmDim[, "item"]     <- paste(prmDim[, "item"], prmDim[, "category"], sep="_")
                                  prmM[,"itemOri"]     <- paste(prmM[,allN[["item"]]])
                                  prmM[,allN[["item"]]]<- paste(prmM[,allN[["item"]]], prmM[,allN[["cat"]]], sep="_")
                               }
    ...
    ### hier jetzt einen PCM-spezifischen Output ausgeben lassen!
                               ret <- eqPartialCredit(ret=ret, cat=cat, eld=eld, prmM = prmM, allN=allN, excludeLinkingDif=excludeLinkingDif)
