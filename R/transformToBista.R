...
### Hotfix fuer bayesianisch
     if (!exists("mat")) { mat <- refPop[match(dimname,  refPop[,1]),] }
...
### 'refPop' Informationstabelle bauen
      ...
      if (isRunM) {
        mat  <- cbind ( model = mod, mat, focusMean = msdFok[1], focusSD = msdFok[2])
      }  else  {
        pv <- NULL
      }
               ...
               refPop <- do.call("rbind", lapply(mods, FUN = function ( mod ) { ### aeussere Schleife: geht ueber modelle
                         ...
                         refL2<- do.call("rbind", lapply(nam2, FUN = function ( dimname ) {
                             ...
                             if ( is.null(weights) ) {
                                ...
                                txt <- capture.output ( msd <- eatRep::repMean ( datL = rex, ID = id, imp = "imp", wgt = colnames(weights)[2], dependent = "value", na.rm = TRUE, verbose = FALSE, progress = FALSE) )
